...
# Define the log file path. It's good practice to put it in a 'logs' or 'data' directory.
# Let's put it in the same 'data' directory where app_config.json is.
_SCRIPT_DIR = os.path.dirname(__file__)
_UNDO_REDO_LOG_FILE = os.path.join(_SCRIPT_DIR, 'data', 'undo_redo_commands.txt')
...
class PaintCanvasWindow(QMainWindow):
    ...
    def __init__(self, background_pixmap, initial_brush_color, initial_brush_size,
        ...
        self.undo_stack = []
        self.undo_index = -1
        self.save_drawing_state()  # İlk durumu kaydet
        self._clear_undo_redo_log()  # Yeni çizim başladığında log dosyasını temizle
    ...
    def clear_all_drawings(self):
        ...
        try:
            ...
            self.save_drawing_state()  # Yeni boş durumu kaydet
            self._clear_undo_redo_log()  # Tüm çizimler temizlendiğinde log dosyasını temizle
    ...
    def undo_drawing(self):
        """Son çizim eylemini geri alır ve eylemi kaydeder."""
        try:
            if self.undo_index > 0:
                self.undo_index -= 1
                self.overlay_image = self.undo_stack[self.undo_index]
                self.update()
                self._log_undo_redo_action("undo")  # Undo hareketini kaydet
            else:
    ...
    def redo_drawing(self):
        """Geri alınan son çizim eylemini tekrar yapar ve eylemi kaydeder."""
        try:
            if self.undo_index < len(self.undo_stack) - 1:
                self.undo_index += 1
                self.overlay_image = self.undo_stack[self.undo_index]
                self.update()
                self._log_undo_redo_action("redo")  # Redo hareketini kaydet
            else:
                print("İleri alınacak başka çizim yok.")
        except Exception as e:
            log_error(f"İleri alma işlemi sırasında hata: {e}", sys.exc_info())

    def _log_undo_redo_action(self, action_type):
        """Önceden tanımlanmış log dosyasına 'undo' veya 'redo' ekler."""
        try:
            with open(_UNDO_REDO_LOG_FILE, 'a', encoding='utf-8') as f:
                f.write(f"{action_type}\n")
            # print(f"'{action_type}' loglandı: {_UNDO_REDO_LOG_FILE}")
        except Exception as e:
            log_error(f"Undo/Redo aksiyonu loglanırken hata: {e}", sys.exc_info())

    def _clear_undo_redo_log(self):
        """Önceden tanımlanmış undo/redo log dosyasının içeriğini temizler."""
        try:
            # Dosyayı açmadan/oluşturmadan önce dizinin var olduğundan emin olun
            os.makedirs(os.path.dirname(_UNDO_REDO_LOG_FILE), exist_ok=True)
            with open(_UNDO_REDO_LOG_FILE, 'w', encoding='utf-8') as f:
                f.truncate(0)  # Dosya içeriğini sil
            # print(f"Undo/Redo log dosyası temizlendi: {_UNDO_REDO_LOG_FILE}")
        except Exception as e:
            log_error(f"Undo/Redo log dosyası temizlenirken hata: {e}", sys.exc_info())
...
class ToolWindow(QWidget):
    ...
    def __init__(self, parent_paint_window, app_config):  # app_config'i parametre olarak al
        ...
        # --- Yeni: Dosyadan İşlem Butonu ---
        self.load_commands_btn = self.findChild(QPushButton, "load_commands_btn")
        if self.load_commands_btn:
            self.load_commands_btn.clicked.connect(self._replay_recorded_commands)  # Metot adı değişti
        else:
            print(
                "Warning: 'load_commands_btn' button not found in pen_tool.ui. File command loading will be unavailable.")
            log_error("UI'da 'load_commands_btn' butonu bulunamadı.")
        # --- Yeni: Dosyadan İşlem Butonu SONU ---
    ...
    def _replay_recorded_commands(self):
        """
        Reads commands from the predefined undo/redo log file
        and executes undo/redo actions on the paint window.
        Commands are expected to be 'undo' or 'redo' on each line.
        """
        try:
            if not os.path.exists(_UNDO_REDO_LOG_FILE):
                QMessageBox.information(self, "Bilgi",
                                        "Kayıtlı komut dosyası bulunamadı. Henüz bir işlem yapılmamış olabilir.")
                return

            with open(_UNDO_REDO_LOG_FILE, 'r', encoding='utf-8') as f:
                commands = f.readlines()
                if not commands:
                    QMessageBox.information(self, "Bilgi", "Kayıtlı komut dosyasında komut bulunamadı.")
                    return

                for line in commands:
                    command = line.strip().lower()
                    if command == "undo":
                        self.paint_window.undo_drawing()
                    elif command == "redo":
                        self.paint_window.redo_drawing()
                    else:
                        print(f"Uyarı: Bilinmeyen komut algılandı: '{line.strip()}'")
                        log_error(f"Bilinmeyen komut dosyadan okundu: {line.strip()}")
            QMessageBox.information(self, "İşlem Tamamlandı", "Kayıtlı komutlar başarıyla uygulandı.")
        except Exception as e:
            QMessageBox.critical(self, "Hata", f"Komutları okurken veya uygularken bir hata oluştu: {e}")
            log_error(f"Kayıtlı komutlar yüklenirken hata: {e}", sys.exc_info())
