...
from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QLabel, QComboBox, QMessageBox, QFrame, QPushButton, QSlider
...
class CursorManager:
    ...
    def load_cursors(cls, json_path):
        ...
        if not cls._base_path:
            print("Hata: İmleç görselleri için temel yol ayarlanmadı. Lütfen CursorManager.set_base_path() çağırın.")
            return
                ...
                for key, cursor_info in cursor_data.items():
                    if isinstance(cursor_info, dict) and "image_path" in cursor_info:
                        ...
                        if not pixmap.isNull():
                            ...
                            print(f"Uyarı: İmleç görseli yüklenemedi: {image_file} (Anahtar: {key}). Varsayılan ok kullanılacak.")
                        ...
                        print(f"Uyarı: Geçersiz imleç tanımı '{key}' JSON'da bulundu. Varsayılan ok kullanılacak.")
                        cls._cursors[key] = QCursor(Qt.ArrowCursor)

                print(f"İmleçler '{json_path}' dosyasından başarıyla yüklendi.")
        except FileNotFoundError:
            print(f"Hata: İmleç dosyası bulunamadı: {json_path}")
        except json.JSONDecodeError:
            print(f"Hata: İmleç dosyası geçersiz JSON formatında: {json_path}")
        except Exception as e:
            print(f"İmleçler yüklenirken bir hata oluştu: {e}")
...
class Ui(QMainWindow):
    ...
    def __init__(self):
        ...
        if not self.app_config:
            QMessageBox.critical(self, "Hata", "Uygulama yapılandırma dosyası yüklenemedi veya eksik.")
            sys.exit(1)
    ...
    def load_app_config(self, config_path):
        ...
        try:
            ...
            print(f"Hata: Uygulama yapılandırma dosyası bulunamadı: {config_path}")
            return None
        except json.JSONDecodeError:
            print(f"Hata: Uygulama yapılandırma dosyası geçersiz JSON formatında: {config_path}")
            return None
        except Exception as e:
            print(f"Uygulama yapılandırması yüklenirken bir hata oluştu: {e}")
            return None
    ...
    def set_application_icon(self, base_dir):
        ...
        if icon_path_relative:
            ...
            if os.path.exists(full_icon_path):
                ...
                print(f"Uyarı: Uygulama simgesi bulunamadı: {full_icon_path}")
        else:
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            if self.resizing:
                ...
                self.setCursor(CursorManager.get_cursor("move_inactive") if self.space_pressed else CursorManager.get_cursor("default")) # JSON'dan imleç çek
            elif self.drawing:
                self.drawing = False
                painter = QPainter(self.overlay_image)  # Paint directly on overlay_image
