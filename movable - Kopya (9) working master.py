                    ...
                    if btn:
                        try:
                            ...
                            if q_color.isValid():
                                ...
                                _debug_print(f"Uyarı: Geçersiz renk değeri '{color_str}' app_config.json'da bulundu.")
                                log_error(f"Geçersiz renk değeri app_config.json'da: {color_str}")
                                if btn: btn.hide()  # Hide button if color is invalid
                        except Exception as e:
                            _debug_print(f"Renk butonu ayarlanırken hata oluştu: {color_str} - {e}")
                            log_error(f"Renk butonu ayarlanırken hata: {color_str} - {e}", sys.exc_info())
                            if btn: btn.hide()  # Hide button on error
                    else:
                        _debug_print(f"Uyarı: {color_button_names[i]} isimli buton UI dosyasında bulunamadı.")
                else:
                    _debug_print(f"Bilgi: app_config.json'da tanımlanan tüm renkler için yeterli buton mevcut değil.")
        else:
            _debug_print("Uyarı: 'pen_colors' anahtarı app_config.json'da bulunamadı. Varsayılan renkler kullanılacak.")
        ...
        try:
            ...
            if event.button() == Qt.LeftButton:
                if self.space_pressed or self.active_tool == "move":
                    ...
                    _debug_print(f"Drawing initiated. Active tool: {self.active_tool}, Brush color alpha: {self.brush_color.alpha()}")
...
class ToolWindow(QWidget):
    ...
    def __init__(self, parent_paint_window, app_config):  # app_config'i parametre olarak al
        ...
        if self.clear_all_btn:
            ...
            _debug_print("Warning: 'clear_all_btn' not found in pen_tool.ui. Clear all functionality will be unavailable.")
        ...
        # --- MANUEL KAYDET/YÜKLE BUTONLARI (Kaldırıldı) ---
        # self.save_drawing_btn = self.findChild(QPushButton, "save_drawing_btn")
        # if self.save_drawing_btn:
        #     self.save_drawing_btn.clicked.connect(self.paint_window.save_drawing_explicitly)
        # else:
        #     _debug_print("Warning: 'save_drawing_btn' button not found in pen_tool.ui.")
        #     log_error("UI'da 'save_drawing_btn' butonu bulunamadı.")

        # self.open_drawing_btn = self.findChild(QPushButton, "open_drawing_btn")
        # if self.open_drawing_btn:
        #     self.open_drawing_btn.clicked.connect(self.paint_window.load_drawing_explicitly)
        # else:
        #     _debug_print("Warning: 'open_drawing_btn' button not found in pen_tool.ui.")
        #     log_error("UI'da 'open_drawing_btn' butonu bulunamadı.")
        # --- MANUEL KAYDET/YÜKLE BUTONLARI SONU ---
        ...
        if self.brushSizeSlider:
            ...
            _debug_print("Warning: 'horizontalSlider' not found in pen_tool.ui. Brush size control will be unavailable.")
    ...
    def _load_auto_saved_drawing(self):
        ...
        try:
            ...
            if not os.path.exists(_AUTO_SAVE_DRAWING_FILE) or os.path.getsize(_AUTO_SAVE_DRAWING_FILE) == 0:
                ...
                _debug_print(f"ToolWindow._load_auto_saved_drawing: Otomatik kayıt dosyası bulunamadı veya boş: {_AUTO_SAVE_DRAWING_FILE}")
                return
