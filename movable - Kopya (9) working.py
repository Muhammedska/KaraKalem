...
from PyQt5.QtGui import QColor, QPen, QPainter, QImage, QPixmap, QCursor, QIcon, qAlpha, qRed, qGreen, qBlue # Added qAlpha, qRed, qGreen, qBlue
...
def _check_qimage_for_visible_content(image: QImage) -> bool:
    ...
    # If the image has an alpha channel, check for any non-zero alpha values.
    if image.hasAlphaChannel():
        for y in range(image.height()):
            for x in range(image.width()):
                pixel_rgb = image.pixel(x, y)  # Returns QRgb (0xAARRGGBB)
                alpha = qAlpha(pixel_rgb)      # Extract alpha component
                if alpha != 0:
                    return True
        return False
    else:
        # If no alpha channel, check for any non-black pixels.
        # This assumes that a "visible" image without transparency will have some color.
        for y in range(image.height()):
            for x in range(image.width()):
                pixel_rgb = image.pixel(x, y)
                # Check if any color component (red, green, blue) is non-zero
...
class Ui(QMainWindow):
    ...
    def __init__(self):
        ...
        if main_pos:
            ...
            _debug_print("Warning: 'main_window_position' not found in app_config.json. Using default position.")
        ...
        try:
            ...
            if load_success and not loaded_image.isNull():
                ...
                if not _check_qimage_for_visible_content(loaded_image):
                    ...
                    _debug_print("Yüklenen çizimin görünür içeriği yok.")
                else:
